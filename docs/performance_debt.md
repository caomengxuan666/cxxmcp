...
| `RequestHandle` wait strategy | Improved | Timeout-only waits use `shared_future::wait_until()` instead of fixed 10ms slices. | Cancellation-token waits still poll because `CancellationToken` is atomic-only; changing that requires a public lifecycle design decision. |
