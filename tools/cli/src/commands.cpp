...
namespace mcp::cli {
    ...
    namespace {
        ...
        core::Result<ParsedCommand> parse_cli11_command(tcb::span<const std::string_view> args);
        ...
        void write_command_usage(std::ostream &out) {
            out << "Usage:\n"
                ...
                << "\n"
                << "Common workflows:\n"
                << "  cxxmcp gateway init-stdio --trust --discover --path /cxxmcp/files --instructions \"Use reviewed file tools only.\" profile.fs filesystem 127.0.0.1 39931 node server.js\n"
                << "  cxxmcp gateway init-http --trust --discover --path /cxxmcp/remote --instructions \"Use reviewed remote tools only.\" --header Authorization \"Bearer token\" profile.remote remote 127.0.0.1 39932 http://127.0.0.1:3000/mcp\n"
                << "  cxxmcp gateway init-all --trust --discover --instructions \"Use imported tools only.\" 127.0.0.1 39940 /cxxmcp/imported\n"
                << "  cxxmcp gateway import-config --trust --discover --instructions \"Use imported tools only.\" ./client-mcp-config.json 127.0.0.1 39940 /cxxmcp/imported\n"
        ...
        class CommandParser {
        public:
            core::Result<ParsedCommand> parse(tcb::span<const std::string_view> args) const {
                return parse_cli11_command(args);
            }

        private:
            core::Result<ParsedCommand> parse_tools(tcb::span<const std::string_view> args) const {
            ...
            core::Result<ParsedCommand> parse_profiles(tcb::span<const std::string_view> args) const {
                if (args.size() == 1 && is_help(args[0])) {
            ...
            core::Result<ParsedCommand> parse_bundle(tcb::span<const std::string_view> args) const {
                if (args.size() == 1 && is_help(args[0])) {
            ...
            core::Result<ParsedCommand> parse_servers(tcb::span<const std::string_view> args) const {
                if (args.size() == 1 && is_help(args[0])) {
            ...
            core::Result<ParsedCommand> parse_capabilities(tcb::span<const std::string_view> args) const {
                if (args.size() == 1 && is_help(args[0])) {
            ...
            core::Result<ParsedCommand> parse_exposures(tcb::span<const std::string_view> args) const {
                if (args.size() == 1 && is_help(args[0])) {
            ...
            core::Result<ParsedCommand> parse_gateway(tcb::span<const std::string_view> args) const {
                if (args.size() == 1 && is_help(args[0])) {
        ...
        bool has_option(tcb::span<const std::string_view> options, std::string_view name) {
            return std::find(options.begin(), options.end(), name) != options.end();
        }

        std::string_view option_value(tcb::span<const std::string_view> options, std::string_view name) {
        ...
        std::string readiness_issue_suggestion(const app::GatewayReadinessIssue &issue,
            ...
            if (issue.code == "endpoint_not_configured") {
                return "configure it with: cxxmcp exposures endpoint " + profile + " 127.0.0.1 <port> /cxxmcp/" +
                       profile;
            }
        ...
        int gateway_status(app::ExposureProfileStore &profiles,
            ...
            if (status.profiles.empty()) {
                out << "No gateway profiles configured\n"
                    << "Next: cxxmcp gateway import-config --trust --discover <config-path> 127.0.0.1 <base-port> /cxxmcp/imported\n";
                return 1;
            }
    ...
    core::Result<int> CommandApp::run(tcb::span<const std::string_view> args, std::ostream &out, std::ostream &err) {
