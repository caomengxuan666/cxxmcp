...
core::Result<McpServerDefinition> find_enabled_server(
  ...
  if (!it->enabled) {
    return std::unexpected(make_runtime_error("cxxmcp server is disabled",
                                              std::string(server_id)));
  }
  if (it->trust == McpServerTrustState::untrusted) {
    return std::unexpected(make_runtime_error("cxxmcp server is untrusted",
                                              std::string(server_id)));
  }
...
struct GatewayRuntimeManager::Impl final {
  ...
  core::Result<core::Unit> start_http_gateway(std::string_view profile_id) {
    ...
    if (!profile) {
      return std::unexpected(make_runtime_error("exposure profile not found",
                                                std::string(profile_id)));
    }
...

    {
      ...
      if (slot && slot->running) {
        return std::unexpected(make_runtime_error("gateway is already running",
                                                  runtime->profile_id));
      }
      slot = runtime;
    }
  ...
  core::Result<core::Unit> stop_http_gateway(std::string_view profile_id) {
    std::shared_ptr<RunningHttpGateway> runtime;
    {
      ...
      if (it == http_gateways.end() || !it->second) {
        return std::unexpected(make_runtime_error("gateway is not running",
                                                  std::string(profile_id)));
      }
      runtime = it->second;
    }
