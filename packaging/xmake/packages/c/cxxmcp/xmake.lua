package("cxxmcp")
    ...
    add_versions("v1.2.3", "48005461f7e6ec5f444ebdcc444202c8a0de42af01360655dde0e7998013dc10")
    ...
    on_load(function (package)
        ...
        local http_enabled = package:config("http") or package:config("websocket")
        if http_enabled then
            package:add("defines", "CXXMCP_ENABLE_HTTP=1")
        end
        if package:config("websocket") then
            package:add("defines", "CXXMCP_ENABLE_WEBSOCKET=1")
        end
        if package:config("auth") then
            package:add("defines", "CXXMCP_ENABLE_AUTH=1")
        end
    ...
    on_install(function (package)
        ...
        local configs = {
            ...
            "-DCXXMCP_ENABLE_HTTP=" .. (http_enabled and "ON" or "OFF"),
            "-DCXXMCP_ENABLE_WEBSOCKET=" .. (package:config("websocket") and "ON" or "OFF"),
