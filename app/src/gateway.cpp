...
core::Error make_gateway_error(protocol::ErrorCode code, std::string message, std::string detail = {}) {
    return core::Error{static_cast<int>(code), std::move(message), std::move(detail)};
}

GatewayReadinessIssue make_readiness_issue(std::string code, std::string message, std::string detail = {}) {
...
std::vector<GatewayReadinessIssue> endpoint_readiness_issues(const HostedEndpoint& endpoint) {
    std::vector<GatewayReadinessIssue> issues;
    if (endpoint.transport != McpServerTransportKind::streamable_http) {
        issues.push_back(make_readiness_issue("endpoint_transport_unsupported",
    ...
    if (endpoint.listen_host.empty() || endpoint.listen_port == 0) {
        issues.push_back(make_readiness_issue("endpoint_not_configured",
                                              "gateway HTTP endpoint is not configured"));
    }
    return issues;
}
...
std::string binding_exposed_name(const CapabilityBinding& binding) {
    ...
    if (binding.namespace_strategy == NamespaceStrategy::none || binding.server_id.empty()) {
...
const DiscoveredCapability* find_capability(const std::vector<DiscoveredCapability>& capabilities,
                                            const CapabilityBinding& binding) {
    const auto it = std::find_if(capabilities.begin(), capabilities.end(), [&](const auto& capability) {
...
protocol::JsonRpcResponse gateway_error_response(const protocol::JsonRpcRequest& request,
...
protocol::JsonRpcResponse gateway_error_response(const protocol::JsonRpcRequest& request, const core::Error& error) {
    return protocol::make_error_response(std::optional<protocol::RequestId>{request.id},
...
GatewayReadinessService::GatewayReadinessService(const ExposureProfileStore& profiles,
...
GatewayReadinessReport GatewayReadinessService::check_profile(std::string_view profile_id) const {
    ...
    const auto* health_reports = health_cache_.has_value() ? &*health_cache_ : nullptr;
    for (const auto& binding : profile->bindings) {
        ...
        if (find_capability(capabilities, binding) == nullptr) {
            report.issues.push_back(make_readiness_issue("capability_not_found",
                                                         "capability binding has no discovered capability",
                                                         binding.id));
        }

        const auto server_it = std::find_if(servers.begin(), servers.end(), [&](const auto& server) {
            return server.id == binding.server_id;
        });
        if (server_it == servers.end()) {
            report.issues.push_back(make_readiness_issue("server_not_found",
                                                         "mcp server not found",
                                                         binding.server_id));
            continue;
        }
        if (!server_it->enabled) {
            report.issues.push_back(make_readiness_issue("server_disabled",
                                                         "cxxmcp server is disabled",
                                                         server_it->id));
        }
        if (server_it->trust == McpServerTrustState::untrusted) {
            report.issues.push_back(make_readiness_issue("server_untrusted",
                                                         "cxxmcp server is untrusted",
                                                         server_it->id));
        }
        if (server_it->trust == McpServerTrustState::blocked) {
            report.issues.push_back(make_readiness_issue("server_blocked",
                                                         "cxxmcp server is blocked",
                                                         server_it->id));
        }

        const bool statically_blocked = !server_it->enabled || server_it->trust != McpServerTrustState::trusted;
        if (health_reports != nullptr && !statically_blocked) {
            const auto health_it = std::find_if(health_reports->begin(), health_reports->end(), [&](const auto& health) {
            ...
            if (health_it != health_reports->end() && !health_it->ready) {
                report.issues.push_back(make_readiness_issue(
                    "server_unready",
                    health_it->error_message.empty() ? "mcp server health check failed" : health_it->error_message,
                    server_it->id));
            }
        }
    }
    ...
    if (report.enabled_binding_count == 0) {
        report.issues.push_back(make_readiness_issue("no_enabled_bindings",
                                                     "no enabled capability bindings configured",
                                                     profile->id));
    }
...
GatewayStatusService::GatewayStatusService(const ExposureProfileStore& profiles,
                                           const CapabilityCatalog& capabilities,
                                           const McpServerStore& servers,
                                           GatewayServerHealthProvider health_provider)
    : profiles_(profiles),
...
GatewayStatusReport GatewayStatusService::check_http_profiles() const {
    ...
    GatewayReadinessService readiness(profiles_, capabilities_, servers_, health_provider_);
    for (const auto& profile : profile_items) {
        ...
        profile_status.http_ready = profile_status.readiness.ready && profile_status.endpoint_issues.empty();
    ...
    status.ready = !status.profiles.empty() && status.ready_profile_count == status.profiles.size();
    return status;
}

GatewayRoutingService::GatewayRoutingService(const ExposureProfileStore& profiles,
                                             const CapabilityCatalog& capabilities,
...
core::Result<ExposureProfile> GatewayRoutingService::get_profile(std::string_view profile_id) const {
    const auto profiles = profiles_.list_exposure_profiles();
    const auto* profile = find_profile(profiles, profile_id);
    if (!profile) {
...
core::Result<std::vector<protocol::ToolDefinition>> GatewayRoutingService::list_tools(
    ...
    for (const auto& binding : profile->bindings) {
        if (!binding.enabled || !binding.policy.enabled || binding.kind != CapabilityKind::tool) {
            continue;
        }
...
core::Result<std::vector<protocol::Prompt>> GatewayRoutingService::list_prompts(std::string_view profile_id) const {
    const auto profiles = profiles_.list_exposure_profiles();
    const auto* profile = find_profile(profiles, profile_id);
    if (!profile) {
        return std::unexpected(make_gateway_error(protocol::ErrorCode::InvalidRequest,
    ...
    for (const auto& binding : profile->bindings) {
        if (!binding.enabled || !binding.policy.enabled || binding.kind != CapabilityKind::prompt) {
            continue;
        }
...
core::Result<std::vector<protocol::Resource>> GatewayRoutingService::list_resources(
        std::string_view profile_id) const {
    const auto profiles = profiles_.list_exposure_profiles();
    const auto* profile = find_profile(profiles, profile_id);
    if (!profile) {
    ...
    for (const auto& binding : profile->bindings) {
        if (!binding.enabled || !binding.policy.enabled || binding.kind != CapabilityKind::resource) {
            continue;
        }
        ...
        if (capability->output_schema.is_object() && capability->output_schema.contains("mimeType") &&
...
core::Result<protocol::ToolResult> GatewayRoutingService::call_tool(std::string_view profile_id,
                                                                    std::string_view exposed_name,
                                                                    protocol::Json arguments) const {
    if (!call_tool_) {
        return std::unexpected(make_gateway_error(protocol::ErrorCode::InternalError,
    ...
    for (const auto& binding : profile->bindings) {
        if (binding.kind != CapabilityKind::tool || binding_exposed_name(binding) != exposed_name) {
            continue;
        }
        if (!binding.enabled || !binding.policy.enabled) {
            return std::unexpected(make_gateway_error(protocol::ErrorCode::PermissionDenied,
...
core::Result<protocol::PromptsGetResult> GatewayRoutingService::get_prompt(std::string_view profile_id,
                                                                           std::string_view exposed_name,
                                                                           protocol::Json arguments) const {
    if (!get_prompt_) {
        return std::unexpected(make_gateway_error(protocol::ErrorCode::InternalError,
    ...
    for (const auto& binding : profile->bindings) {
        if (binding.kind != CapabilityKind::prompt || binding_exposed_name(binding) != exposed_name) {
            continue;
        }
        if (!binding.enabled || !binding.policy.enabled) {
            return std::unexpected(make_gateway_error(protocol::ErrorCode::PermissionDenied,
                                                      "prompt binding is disabled",
                                                      std::string(exposed_name)));
        }

        return get_prompt_(binding.server_id,
...
core::Result<protocol::ResourcesReadResult> GatewayRoutingService::read_resource(std::string_view profile_id,
    ...
    for (const auto& binding : profile->bindings) {
        ...
        if (!binding.enabled || !binding.policy.enabled) {
            return std::unexpected(make_gateway_error(protocol::ErrorCode::PermissionDenied,
                                                      "resource binding is disabled",
                                                      std::string(uri)));
        }

        return read_resource_(binding.server_id, protocol::ResourcesReadParams{.uri = capability->uri});
    }

    return std::unexpected(make_gateway_error(protocol::ErrorCode::ResourceNotFound,
                                              "resource binding not found",
                                              std::string(uri)));
}

GatewayRequestHandler::GatewayRequestHandler(const GatewayRoutingService& routing, std::string profile_id)
    : routing_(routing),
      profile_id_(std::move(profile_id)) {}

core::Result<protocol::JsonRpcResponse> GatewayRequestHandler::handle(const protocol::JsonRpcRequest& request) const {
    if (request.method == protocol::InitializeMethod) {
        ...
        result["capabilities"] = protocol::Json{
            ...
            {"resources", protocol::Json{{"listChanged", false}, {"subscribe", false}}},
        };
    ...
    if (request.method == "prompts/get") {
        ...
        const auto result = routing_.get_prompt(profile_id_, params->name, params->arguments);
        if (!result) {
    ...
    if (request.method == "tools/call") {
        ...
        const auto result = routing_.call_tool(profile_id_,
                                               request.params.at("name").get<std::string>(),
                                               std::move(arguments));
        if (!result) {
            return gateway_error_response(request, result.error());
        }
