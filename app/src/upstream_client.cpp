...
core::Result<client::HttpTransportOptions> http_transport_options_from_url(const std::string& url) {
    constexpr std::string_view kHttpScheme = "http://";
    if (!url.starts_with(kHttpScheme)) {
        if (url.starts_with("https://")) {
            return std::unexpected(make_runtime_error("https upstream transport is not supported in this build", url));
        }
        return std::unexpected(make_runtime_error("http upstream url must start with http://", url));
    }
    ...
    if (host_port.empty()) {
        return std::unexpected(make_runtime_error("http upstream url must include a host", url));
    }
    ...
    if (host_port.starts_with('[')) {
        ...
        if (close == std::string_view::npos) {
            return std::unexpected(make_runtime_error("invalid ipv6 http upstream url", url));
        }
        ...
        if (close + 1 < host_port.size()) {
            ...
            if (port_text.empty()) {
                return std::unexpected(make_runtime_error("http upstream port must not be empty", url));
            }
            try {
                port = std::stoi(std::string(port_text));
            } catch (const std::exception&) {
                return std::unexpected(make_runtime_error("http upstream port must be numeric", url));
            }
        }
    } else {
    ...
    if (port <= 0 || port > 65535) {
        return std::unexpected(make_runtime_error("http upstream port out of range", url));
    }
...
core::Result<std::unique_ptr<client::Transport>> make_client_transport_for_server(
    ...
    if (!server.enabled) {
        return std::unexpected(make_runtime_error("cxxmcp server is disabled", server.id));
    }

    switch (server.transport) {
    case McpServerTransportKind::stdio:
        return std::make_unique<client::ProcessStdioTransport>(client::ProcessStdioTransportOptions{
    ...
    case McpServerTransportKind::streamable_http: {
        if (server.http.url.empty()) {
            return std::unexpected(make_runtime_error("http cxxmcp server url must not be empty", server.id));
        }
    ...
    case McpServerTransportKind::legacy_sse:
        return std::unexpected(make_runtime_error("legacy sse upstream transport is not supported", server.id));
    }

    return std::unexpected(make_runtime_error("unknown cxxmcp server transport", server.id));
}
