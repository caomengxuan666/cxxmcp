...
core::Error make_client_config_error(std::string message, std::string detail = {}) {
    return core::Error{1, std::move(message), std::move(detail)};
}

std::string endpoint_url(const HostedEndpoint& endpoint) {
    std::string path = endpoint.path.empty() ? std::string("/mcp") : endpoint.path;
    if (!path.starts_with('/')) {
        path.insert(path.begin(), '/');
    }
    return "http://" + endpoint.listen_host + ":" + std::to_string(endpoint.listen_port) + path;
}
...
core::Result<ExposureProfile> find_profile(const ExposureProfileStore& profiles, std::string_view profile_id) {
    ...
    if (profile_it == configured_profiles.end()) {
        return std::unexpected(make_client_config_error("exposure profile not found", std::string(profile_id)));
    }
    return *profile_it;
}
...
core::Result<Json> http_server_config_for_profile(const ExposureProfile& profile) {
    if (profile.endpoint.transport != McpServerTransportKind::streamable_http) {
        return std::unexpected(make_client_config_error("exposure endpoint transport must be streamable_http",
                                                       profile.id));
    }
    if (profile.endpoint.listen_host.empty()) {
        return std::unexpected(make_client_config_error("exposure endpoint host is not configured",
                                                       profile.id));
    }
    if (profile.endpoint.listen_port == 0) {
        return std::unexpected(make_client_config_error("exposure endpoint port is not configured",
                                                       profile.id));
    }
...
std::string prefixed_server_name(std::string_view prefix, std::string_view profile_id) {
    if (prefix.empty()) {
...
GatewayClientConfigService::GatewayClientConfigService(const ExposureProfileStore& profiles)
    : profiles_(profiles) {}

core::Result<Json> GatewayClientConfigService::make_http_client_config(std::string_view profile_id,
    ...
    const auto name = server_name.empty() ? profile->id : std::string(server_name);
...
core::Result<Json> GatewayClientConfigService::make_all_http_client_configs(
    ...
    if (configured_profiles.empty()) {
        return std::unexpected(make_client_config_error("no gateway profiles configured"));
    }
    ...
    for (const auto& profile : configured_profiles) {
        ...
        servers[prefixed_server_name(server_name_prefix, profile.id)] = *server_config;
    }
...
core::Result<Json> GatewayClientConfigService::make_ready_http_client_configs(
    ...
    if (servers.empty()) {
        return std::unexpected(make_client_config_error("no ready HTTP gateway profiles configured"));
    }

    return Json{{"mcpServers", std::move(servers)}};
}

core::Result<Json> GatewayClientConfigService::make_stdio_client_config(std::string_view profile_id,
    ...
    if (command.empty()) {
        return std::unexpected(make_client_config_error("stdio client config command is not configured",
                                                       std::string(profile_id)));
    }

    const auto name = server_name.empty() ? profile->id : std::string(server_name);
    return client_config_with_server(name,
                                     Json{
