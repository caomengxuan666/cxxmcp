...
std::vector<ToolDescriptor> MemoryToolCatalog::list() const {
    return tools_;
}
...
core::Result<core::Unit> MemoryToolCatalog::add(ToolDescriptor tool) {
    ...
    if (tool.definition.name.empty()) {
        return std::unexpected(make_app_error("tool definition name must not be empty"));
    }
...
core::Result<core::Unit> MemoryToolCatalog::update_policy(std::string tool_id, Policy policy) {
    const auto it = std::find_if(tools_.begin(), tools_.end(), [&](const auto& existing) {
        return existing.id == tool_id;
    });
    if (it == tools_.end()) {
        return std::unexpected(make_app_error("tool not found", std::move(tool_id)));
    }
...
MemoryResourceCatalog::MemoryResourceCatalog(std::vector<ResourceDescriptor> resources)
...
core::Result<core::Unit> MemoryResourceCatalog::add(ResourceDescriptor resource) {
    ...
    const auto it = std::find_if(resources_.begin(), resources_.end(), [&](const auto& existing) {
        return existing.id == resource.id;
    });
    if (it != resources_.end()) {
        return std::unexpected(make_app_error("resource already exists", resource.id));
    }
...
core::Result<core::Unit> MemoryPromptCatalog::add(PromptDescriptor prompt) {
    ...
    const auto it = std::find_if(prompts_.begin(), prompts_.end(), [&](const auto& existing) {
        return existing.id == prompt.id;
    });
    if (it != prompts_.end()) {
...
core::Result<ExportBundle> JsonImportExportService::import_bundle(const std::filesystem::path& path) {
    std::ifstream input(path);
    if (!input.is_open()) {
        return std::unexpected(make_app_error("failed to open bundle", path.string()));
    }
    ...
    try {
        ...
        return std::unexpected(make_app_error("failed to parse bundle", exception.what()));
    }
...
core::Result<core::Unit> JsonImportExportService::export_bundle(const ExportBundle& bundle,
    ...
    if (!parent.empty()) {
        ...
        if (ec) {
            return std::unexpected(make_app_error("failed to create export directory", ec.message()));
        }
    }

    std::ofstream output(path);
    if (!output.is_open()) {
        return std::unexpected(make_app_error("failed to open bundle for writing", path.string()));
    }

    output << to_json(bundle).dump(2);
    if (!output.good()) {
        return std::unexpected(make_app_error("failed to write bundle", path.string()));
    }
