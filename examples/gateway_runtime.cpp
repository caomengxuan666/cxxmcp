...
int main() {
    ...
    require(wait_for_gateway(39981, "/cxxmcp/example"), "gateway did not become reachable");
    ...
    require(client.notify_initialized().has_value(), "gateway initialized notification failed");
    ...
    require(prompts.has_value() && !prompts->empty(), "gateway prompts/list failed");

    const auto resources = client.list_resources();
    require(resources.has_value() && !resources->empty(), "gateway resources/list failed");

    const auto call = client.call_raw(tools->front().name, mcp::protocol::Json{{"value", "hello"}});
    require(call.has_value(), "gateway tools/call failed");

    const auto prompt = client.get_prompt(prompts->front().name, mcp::protocol::Json{{"text", "hello"}});
    require(prompt.has_value() && !prompt->messages.empty(), "gateway prompts/get failed");

    const auto resource = client.read_resource(resources->front().uri);
    require(resource.has_value() && !resource->contents.empty(), "gateway resources/read failed");
