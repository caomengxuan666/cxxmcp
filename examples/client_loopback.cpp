...
int main() {
    try {
        ...
        require(tools.has_value() && tools->size() == 2, "client list_tools failed");

        const auto all_tools = client.list_all_tools();
        require(all_tools.has_value() && all_tools->size() == 2, "client list_all_tools failed");

        const auto echoed = client.call_raw("echo", mcp::protocol::Json{{"value", "hello"}});
        require(echoed.has_value(), "client call_tool failed");

        const auto shouted = client.call_raw("shout", mcp::protocol::Json{{"value", "hello"}});
        require(shouted.has_value(), "client typed call_tool failed");

        const auto prompts = client.list_prompts();
        require(prompts.has_value() && prompts->size() == 1, "client list_prompts failed");

        const auto all_prompts = client.list_all_prompts();
        require(all_prompts.has_value() && all_prompts->size() == 1, "client list_all_prompts failed");

        const auto prompt = client.get_prompt("summarize", mcp::protocol::Json{{"text", "hello"}});
        require(prompt.has_value() && !prompt->messages.empty(), "client get_prompt failed");

        const auto resources = client.list_resources();
        require(resources.has_value() && resources->size() == 1, "client list_resources failed");

        const auto all_resources = client.list_all_resources();
        require(all_resources.has_value() && all_resources->size() == 1, "client list_all_resources failed");

        const auto resource = client.read_resource("file:///workspace/README.md");
        require(resource.has_value() && !resource->contents.empty(), "client read_resource failed");

        const auto templates = client.list_resource_templates();
        require(templates.has_value() && templates->size() == 1, "client list_resource_templates failed");

        const auto all_templates = client.list_all_resource_templates();
        require(all_templates.has_value() && all_templates->size() == 1, "client list_all_resource_templates failed");

        const auto completion = client.complete(mcp::protocol::Json{{"prefix", "pr"}});
        require(completion.has_value(), "client complete failed");
        require(completion->at("completion").at("values").size() == 2, "client complete payload mismatch");

        const auto sample = client.create_message(mcp::protocol::Json{{"prompt", "write a summary"}});
        ...
        require(client.notify_initialized().has_value(), "client notify_initialized failed");
        require(client.notify_cancelled(std::int64_t{7}, "done").has_value(), "client notify_cancelled failed");
        require(client.notify_progress(std::int64_t{8}, 0.5, 1.0).has_value(), "client notify_progress failed");
        require(client.notify_roots_list_changed().has_value(), "client notify_roots_list_changed failed");
        ...
        require(tool_notifications == 1, "client tool notification handler not called");
        require(prompt_notifications == 1, "client prompt notification handler not called");
        require(resource_notifications == 1, "client resource notification handler not called");
        require(root_notifications == 1, "client roots notification handler not called");
        require(updated_uris.size() == 1 && updated_uris.front() == "file:///workspace/README.md",
                "client resource updated handler mismatch");
        require(raw_methods.size() == 6, "client raw notification handler mismatch");
