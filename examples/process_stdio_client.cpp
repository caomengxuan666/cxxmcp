...
int main() {
  try {
    ...
    mcp::client::McpClientSession session(
        std::make_unique<mcp::client::ProcessStdioTransport>(
    ...
    require(prompt.has_value() && !prompt->messages.empty(),
            "process session get_prompt failed");
    ...
    require(resource.has_value() && !resource->contents.empty(),
            "process session read_resource failed");
    ...
    const auto completion =
        session.client().complete(mcp::protocol::Json{{"prefix", "pr"}});
    require(completion.has_value(), "process session complete failed");
    require(completion->at("completion").at("values").size() == 2,
            "process session completion payload mismatch");

    const auto sampling = session.client().create_message(
    ...
    require(sampling->at("role") == "assistant",
            "process session sampling role mismatch");

    const auto health =
        session.client().raw_request(mcp::protocol::JsonRpcRequest{
            .method = "example/health",
            .params = mcp::protocol::Json::object(),
            .id = std::int64_t{42},
        });
    require(health.has_value() && health->value("ok", false),
            "process session raw_request failed");

    require(session.client().set_level("info").has_value(),
            "process session set_level failed");
