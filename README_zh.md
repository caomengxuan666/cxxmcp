...
client 和 server 的现代 C++17 SDK。

核心 SDK 表面保持稳定、窄、可打包：`protocol`、`transport`、`handler`、
`peer`、`service`、`client` 和 `server` 是主要库层。
...
Public SDK headers 和 package targets 面向 C++17。可选非 SDK targets、
examples 和 tests 需要 C++20。
...
这些路径只构建 C++17 SDK targets，并关闭可选工具层。当前 vcpkg port
是用于本仓库 checkout 的 overlay port：
...
[HTTP transport backend evidence](docs/http_transport_backend_evidence.md)。
...
- 兼容和低层示例：`stdio_server`、`client_loopback`
...
示例分类维护在 [Examples](docs/examples.md)。Runtime 和 gateway tooling 单独记录在
...
task/cancellation、plugin/adapters、gateway runtime，以及 app service
management。
