...
mcp::app::CapabilityBinding capability_binding(const mcp::app::DiscoveredCapability& capability) {
    return mcp::app::CapabilityBinding{
...
mcp::app::ExposureProfile dev_profile(std::vector<mcp::app::CapabilityBinding> bindings) {
...
private:
    void handle(const httplib::Request& request, httplib::Response& response) {
        ...
        if (rpc_request->method == "tools/call") {
            const auto serialized = mcp::protocol::serialize_response(mcp::protocol::JsonRpcResponse{
                .id = rpc_request->id,
                .result = mcp::protocol::tool_result_to_json(mcp::protocol::ToolResult{
                    .content = {mcp::protocol::ContentBlock{
                        .type = "text",
                        .text = "http upstream",
                        .data = Json::object(),
                    }},
                    .structured_content = rpc_request->params.value("arguments", Json::object()),
                    .is_error = false,
                }),
            });
            response.set_content(*serialized, "application/json");
...
void test_gateway_lists_profile_bound_tools() {
    mcp::app::MemoryExposureProfileStore profiles({dev_profile({read_file_binding()})});
    mcp::app::MemoryCapabilityCatalog capabilities({read_file_capability()});
    mcp::app::GatewayRoutingService gateway(
        profiles,
        capabilities,
        [](std::string_view, const mcp::protocol::ToolCall&) -> mcp::core::Result<mcp::protocol::ToolResult> {
    ...
    require(tools->front().name == "filesystem.read_file", "gateway exposed tool name mismatch");
    require(tools->front().description == "Read a local file", "gateway tool description mismatch");
    require(tools->front().input_schema.at("type") == "object", "gateway tool schema mismatch");
}
...
void test_gateway_routes_call_to_upstream_tool_name() {
    ...
    mcp::app::GatewayRoutingService gateway(
        ...
        [&](std::string_view server_id,
            const mcp::protocol::ToolCall& call) -> mcp::core::Result<mcp::protocol::ToolResult> {
    ...
    const auto result = gateway.call_tool("profile.dev", "filesystem.read_file", Json{{"path", "README.md"}});
    require(result.has_value(), "gateway call_tool should succeed");
    require(recorded.server_id == "server.filesystem", "gateway upstream server mismatch");
    require(recorded.tool_name == "read_file", "gateway upstream tool mismatch");
    require(recorded.arguments.at("path") == "README.md", "gateway upstream arguments mismatch");
    require(result->content.front().text == "ok", "gateway result mismatch");
}

void test_disabled_binding_is_not_exposed() {
    mcp::app::MemoryExposureProfileStore profiles({dev_profile({read_file_binding(false)})});
    mcp::app::MemoryCapabilityCatalog capabilities({read_file_capability()});
    mcp::app::GatewayRoutingService gateway(
        profiles,
        capabilities,
    ...
    const auto called = gateway.call_tool("profile.dev", "filesystem.read_file", Json::object());
    require(!called.has_value(), "disabled binding call should fail");
    require(called.error().code == static_cast<int>(mcp::protocol::ErrorCode::PermissionDenied),
...
void test_gateway_readiness_reports_ready_profile() {
    ...
    require(report.binding_count == 1, "gateway readiness binding count mismatch");
    require(report.enabled_binding_count == 1, "gateway readiness enabled binding count mismatch");
    require(report.issues.empty(), "gateway readiness ready profile should not have issues");
}
...
void test_gateway_readiness_reports_runtime_unready_server() {
    ...
    require(!report.ready, "gateway readiness should report runtime-unready profile");
    require(report.issues.size() == 1, "gateway runtime readiness issue count mismatch");
    require(report.issues[0].code == "server_unready", "gateway runtime readiness issue code mismatch");
...
void test_gateway_readiness_reports_upstream_issues() {
    ...
    require(report.issues[0].code == "capability_not_found", "gateway readiness capability issue mismatch");
    require(report.issues[1].code == "server_untrusted", "gateway readiness server issue mismatch");
}
...
void test_gateway_handler_lists_tools_for_downstream_request() {
    ...
    require(response->result.has_value(), "gateway handler tools/list should return result");
    require(response->result->at("tools").size() == 1, "gateway handler tool count mismatch");
    require(response->result->at("tools").front().at("name") == "filesystem.read_file",
            "gateway handler exposed tool mismatch");
    require(std::get<std::int64_t>(*response->id) == 7, "gateway handler response id mismatch");
}
...
void test_gateway_handler_initialize_and_ping() {
    ...
    require(initialized->result->at("protocolVersion") == std::string(mcp::protocol::McpProtocolVersion),
    ...
    require(initialized->result->at("instructions") == "Expose reviewed tools only.",
...
void test_gateway_handler_lists_profile_bound_prompts_and_resources() {
    ...
    mcp::app::MemoryExposureProfileStore profiles({dev_profile({capability_binding(prompt), capability_binding(resource)})});
    ...
    require(prompts->result.has_value(), "gateway handler prompts/list result missing");
    require(prompts->result->at("prompts").size() == 1, "gateway handler prompt count mismatch");
    require(prompts->result->at("prompts").front().at("name") == "filesystem.summarize",
    ...
    require(resources.has_value(), "gateway handler resources/list should succeed");
    require(resources->result.has_value(), "gateway handler resources/list result missing");
    require(resources->result->at("resources").size() == 1, "gateway handler resource count mismatch");
    require(resources->result->at("resources").front().at("uri") == "file:///workspace/README.md",
...
void test_gateway_handler_routes_tools_call_request() {
    ...
    require(response->result.has_value(), "gateway handler tools/call should return result");
    require(response->result->at("content").front().at("text") == "routed", "gateway handler result mismatch");
    require(recorded.server_id == "server.filesystem", "gateway handler server route mismatch");
    require(recorded.tool_name == "read_file", "gateway handler tool route mismatch");
    require(recorded.arguments.at("path") == "README.md", "gateway handler args mismatch");
}
...
void test_gateway_handler_rejects_invalid_tools_call_params() {
    ...
    require(response.has_value(), "invalid request should produce JSON-RPC error response");
    require(response->error.has_value(), "invalid tools/call should return error");
    require(response->error->code == static_cast<int>(mcp::protocol::ErrorCode::InvalidRequest),
...
void test_stdio_gateway_routes_tools_call_to_child_process_server() {
    ...
    mcp::app::MemoryExposureProfileStore profiles({dev_profile({child_echo_binding()})});
    ...
    const auto served = mcp::app::run_stdio_gateway(gateway, "profile.dev", input, output);
    ...
    require(response->result.has_value(), "stdio gateway tools/call should return result");
...
void test_stdio_gateway_routes_prompt_and_resource_requests_to_child_process_server() {
    ...
    mcp::app::MemoryExposureProfileStore profiles(
        {dev_profile({capability_binding(prompt), capability_binding(resource)})});
    mcp::app::MemoryCapabilityCatalog capabilities({prompt, resource});
    mcp::app::GatewayRoutingService gateway(
        profiles,
        capabilities,
    ...
    std::ostringstream output;
    const auto served = mcp::app::run_stdio_gateway(gateway, "profile.dev", input, output);
    require(served.has_value(), "stdio gateway should serve prompt/resource requests");
    ...
    require(prompt_response.has_value(), "stdio gateway prompt response should parse");
    require(prompt_response->result.has_value(), "stdio gateway prompts/get should return result");
    require(prompt_response->result->at("messages").front().at("content").at("text") == "Summarize hello",
            "stdio gateway upstream prompt text mismatch");

    const auto resource_response = mcp::protocol::parse_response(resource_line);
    require(resource_response.has_value(), "stdio gateway resource response should parse");
    require(resource_response->result.has_value(), "stdio gateway resources/read should return result");
    require(resource_response->result->at("contents").front().at("text") == "hello from readme",
...
void test_gateway_rejects_untrusted_upstream_server() {
    ...
    const auto result = caller("server.child", mcp::protocol::ToolCall{
                                                   .name = "echo",
                                                   .arguments = Json{{"message", "blocked"}},
                                               });

    require(!result.has_value(), "untrusted upstream server call should fail");
    require(result.error().message == "cxxmcp server is untrusted", "untrusted upstream error mismatch");
}
...
void test_gateway_rejects_blocked_upstream_server() {
    ...
    require(result.error().message == "cxxmcp server is blocked", "blocked upstream error mismatch");
}

void test_http_gateway_exposes_profile_tools() {
    auto profile = dev_profile({read_file_binding()});
    profile.endpoint.listen_port = 39917;
    profile.endpoint.path = "/mcp/http-test";
    ...
    std::thread server_thread([&]() {
        const auto served = transport.start([&](const mcp::protocol::JsonRpcRequest& request,
    ...
    require(wait_for_http_gateway(profile.endpoint.listen_port, profile.endpoint.path),
    ...
    require(tools->front().name == "filesystem.read_file", "http gateway exposed tool mismatch");
    ...
    require(called->content.front().text == "http routed", "http gateway call result mismatch");
    require(recorded.server_id == "server.filesystem", "http gateway routed server mismatch");
    require(recorded.tool_name == "read_file", "http gateway upstream tool mismatch");
    require(recorded.arguments.at("path") == "README.md", "http gateway args mismatch");

    transport.stop();
    if (server_thread.joinable()) {
        server_thread.join();
    }
    require(!server_error.has_value(), "http gateway transport should stop cleanly");
}
...
void test_http_gateway_routes_to_http_upstream_server() {
    ...
    profile.endpoint.path = "/mcp/http-upstream-test";
    ...
    require(called.has_value(), "http gateway should route tools/call to http upstream");
    require(called->content.front().text == "http upstream", "http upstream result text mismatch");
    require(called->structured_content.has_value(), "http upstream structured content missing");
    require(called->structured_content->at("value") == 42, "http upstream arguments mismatch");
    require(upstream.authorization_header_seen(), "http gateway should send configured upstream headers");

    transport.stop();
    if (server_thread.joinable()) {
        server_thread.join();
    }
    require(!server_error.has_value(), "http upstream gateway transport should stop cleanly");
}

void test_gateway_runtime_manager_starts_and_stops_http_gateway() {
    auto profile = dev_profile({read_file_binding()});
    profile.endpoint.listen_port = 39931;
    profile.endpoint.path = "/mcp/runtime-manager";
    ...
    mcp::app::GatewayRuntimeManager runtime(
        ...
        [&](std::string_view server_id,
            const mcp::protocol::ToolCall& call) -> mcp::core::Result<mcp::protocol::ToolResult> {
            require(server_id == "server.filesystem", "runtime manager server id mismatch");
    ...
    require(snapshot.front().running, "runtime manager should report running gateway");
    require(snapshot.front().endpoint.listen_port == 39931, "runtime manager endpoint port mismatch");
    ...
    require(tools->front().name == "filesystem.read_file", "managed http gateway tool mismatch");

    const auto stopped = runtime.stop_http_gateway("profile.dev");
    require(stopped.has_value(), "runtime manager should stop gateway");
    require(wait_for_http_gateway_shutdown(profile.endpoint.listen_port, profile.endpoint.path),
            "managed http gateway should stop");

    const auto stopped_snapshot = runtime.list_http_gateways();
    require(stopped_snapshot.size() == 1, "stopped runtime manager snapshot count mismatch");
    require(!stopped_snapshot.front().running, "runtime manager should report stopped gateway");
}
...
void test_public_gateway_runtime_builder_smoke() {
    ...
    require(wait_for_http_gateway(39951, "/mcp/public"), "public gateway should become reachable");
    ...
    require(wait_for_http_gateway_shutdown(39951, "/mcp/public"),
...
int main() {
    const std::vector<std::pair<std::string_view, void (*)()>> tests = {
        {"gateway lists profile-bound tools", test_gateway_lists_profile_bound_tools},
        ...
        {"gateway readiness reports ready profile", test_gateway_readiness_reports_ready_profile},
        {"gateway readiness reports runtime-unready server",
         test_gateway_readiness_reports_runtime_unready_server},
        {"gateway readiness reports upstream issues", test_gateway_readiness_reports_upstream_issues},
        {"gateway handler lists tools", test_gateway_handler_lists_tools_for_downstream_request},
        {"gateway handler initialize and ping", test_gateway_handler_initialize_and_ping},
        {"gateway handler lists prompts and resources",
         test_gateway_handler_lists_profile_bound_prompts_and_resources},
        {"gateway handler routes tools/call", test_gateway_handler_routes_tools_call_request},
        {"gateway handler rejects invalid tools/call", test_gateway_handler_rejects_invalid_tools_call_params},
        ...
        {"gateway rejects untrusted upstream server", test_gateway_rejects_untrusted_upstream_server},
        {"gateway rejects blocked upstream server", test_gateway_rejects_blocked_upstream_server},
        {"http gateway exposes profile tools", test_http_gateway_exposes_profile_tools},
        {"http gateway routes to http upstream server", test_http_gateway_routes_to_http_upstream_server},
        {"gateway runtime manager starts and stops http gateway",
         test_gateway_runtime_manager_starts_and_stops_http_gateway},
        {"public gateway runtime builder smoke", test_public_gateway_runtime_builder_smoke},
    };
