    ...
    explicit CliHarness(std::vector<mcp::app::ToolDescriptor> tools = {tool("tool.echo", "echo", true)},
        ...
        : tools_(std::move(tools)),
          ...
          cli_(mcp::cli::CommandServices{.management = management_, .tools = tools_, .profiles = profiles_,
                                         .bundles = bundles_, .servers = servers_,
...
void test_doctor_reports_ready_runtime_as_json() {
    ...
    require(harness.run({"exposures", "endpoint", "profile.dev", "127.0.0.1", "39919", "mcp/dev"}) == 0,
...
void test_creates_and_lists_exposure_profiles() {
    ...
    harness.out.str({});
    harness.out.clear();
    exit_code = harness.run({"exposures", "endpoint", "profile.dev", "127.0.0.1", "39919", "mcp/dev"});
    require(exit_code == 0, "exposures endpoint exit code mismatch");
    require(harness.out.str() == "Configured exposure endpoint profile.dev at http://127.0.0.1:39919/mcp/dev\n",
    ...
    require(harness.out.str() == "profile.dev\tDev\t127.0.0.1:39919\t/mcp/dev\t0 binding(s)\n",
...
void test_gateway_lists_and_inspects_profiles() {
    ...
    require(harness.out.str() == "profile.dev\tDev\t127.0.0.1:39919\t/mcp/dev\t1 binding(s)\n",
    ...
    require(harness.out.str().find("endpoint: http://127.0.0.1:39919/mcp/dev\n") != std::string::npos,
...
void test_creates_and_configures_exposure_profile_as_json() {
    ...
    require(json["endpoint"]["path"] == "/mcp/dev", "json exposures endpoint path mismatch");
...
void test_writes_gateway_client_config() {
    ...
    require(json["mcpServers"]["dev-gateway"]["url"] == "http://127.0.0.1:39919/mcp/dev",
...
void test_gateway_client_config_requires_ready_profile() {
    ...
    require(harness.run({"exposures", "endpoint", "profile.empty", "127.0.0.1", "39919", "mcp/empty"}) == 0,
            "exposure endpoint failed");
...
void test_writes_gateway_all_client_config() {
    ...
    require(harness.run({"exposures", "endpoint", "profile.ops", "127.0.0.1", "39920", "/mcp/ops"}) == 0,
    ...
    require(json["mcpServers"]["local.profile.dev"]["url"] == "http://127.0.0.1:39919/mcp/dev",
            "gateway client-config-all first url mismatch");
    require(json["mcpServers"]["local.profile.ops"]["url"] == "http://127.0.0.1:39920/mcp/ops",
...
void test_gateway_all_client_config_requires_ready_profiles() {
    ...
    require(harness.run({"exposures", "endpoint", "profile.empty", "127.0.0.1", "39920", "mcp/empty"}) == 0,
...
void test_initializes_gateway_profile_from_server_as_json() {
    ...
    require(json["endpoint"]["url"] == "http://127.0.0.1:39919/mcp/dev",
...
void test_gateway_init_reuses_existing_profile() {
    ...
    harness.out.clear();
    const auto exit_code =
        harness.run({"gateway", "init", "profile.dev", "filesystem", "127.0.0.1", "39920", "mcp/updated"});
    ...
    require(profiles.front().endpoint.path == "/mcp/updated", "gateway init existing profile path mismatch");
...
void test_gateway_init_without_capabilities_reports_discovery_hint() {
    ...
    const auto exit_code =
        harness.run({"gateway", "init", "profile.dev", "filesystem", "127.0.0.1", "39919", "mcp/dev"});
...
void test_initializes_gateway_profile_with_stdio_server_onboarding() {
    ...
    require(harness.out.str().find("Endpoint: http://127.0.0.1:39941/mcp/child\n") !=
                std::string::npos,
...
void test_initializes_gateway_profile_with_stdio_server_onboarding_as_json() {
    ...
    require(json["endpoint"]["url"] == "http://127.0.0.1:39941/mcp/child",
...
void test_initializes_gateway_profile_with_http_server_onboarding() {
    ...
    require(harness.out.str().find("Endpoint: http://127.0.0.1:39942/mcp/remote\n") !=
                std::string::npos,
...
void test_initializes_gateway_profile_with_http_server_onboarding_as_json() {
    ...
    require(json["endpoint"]["url"] == "http://127.0.0.1:39942/mcp/remote",
...
void test_gateway_onboarding_one_shot_flow() {
    ...
    require(config["mcpServers"]["dev-gateway"]["url"] == "http://127.0.0.1:39931/mcp/cli",
...
void test_gateway_init_all_initializes_discovered_servers_as_json() {
    ...
    require(json["profiles"][0]["url"] == "http://127.0.0.1:39950/mcp/imported/filesystem",
    ...
    require(profile->endpoint.path == "/mcp/imported/filesystem",
...
void test_gateway_init_all_prints_ready_only_next_steps() {
    ...
    const auto exit_code = harness.run({"gateway", "init-all", "127.0.0.1", "39950", "mcp/imported"});
...
void test_gateway_import_config_trusts_discovers_and_initializes_as_json() {
  ...
  "mcpServers": {
    ...
    require(json["profiles"][0]["url"] == "http://127.0.0.1:39970/mcp/imported/filesystem",
...
void test_checks_all_gateway_profiles() {
    ...
    require(harness.run({"exposures", "endpoint", "profile.ready", "127.0.0.1", "39919", "mcp/ready"}) == 0,
            "ready exposure endpoint failed");
...
void test_gateway_status_summarizes_http_client_readiness() {
    ...
    require(harness.out.str().find("profile.ready\tready\thttp://127.0.0.1:39919/mcp/ready\t1 enabled binding(s)\n") !=
                std::string::npos,
...
void test_gateway_status_reports_json() {
    ...
    require(json["profiles"][0]["endpoint"]["url"] == "http://127.0.0.1:39919/mcp/ready",
...
void test_previews_gateway_exposed_capabilities() {
    ...
    require(harness.out.str().find("profile.dev\tready\thttp://127.0.0.1:39919/mcp/dev\n") !=
                std::string::npos,
...
void test_gateway_serve_http_end_to_end() {
    ...
    require(wait_for_http_gateway(port, "/mcp/cli"), "gateway serve-http should become reachable");
...
void test_cli_binary_gateway_serve_http_proxies_stdio_child() {
    ...
    require(wait_for_http_gateway(port, "/mcp/http"), "binary http proxy gateway should become reachable");
...
void test_cli_binary_gateway_serve_http_isolates_profiles() {
    ...
    require(wait_for_http_gateway(alpha_port, "/mcp/alpha"),
            "binary http multi-profile alpha gateway should become reachable");
    require(wait_for_http_gateway(beta_port, "/mcp/beta"),
...
void test_cli_binary_import_init_all_and_serve_all_child() {
    ...
    require(initialized["profiles"][0]["url"] ==
                ("http://127.0.0.1:" + std::to_string(port) + "/mcp/imported/child"),
    ...
    require(wait_for_http_gateway(port, "/mcp/imported/child"),
...
void test_cli_binary_gateway_import_config_initializes_child() {
    ...
    require(imported["profiles"][0]["url"] ==
                ("http://127.0.0.1:" + std::to_string(port) + "/mcp/one-step/child"),
...
void test_cli_binary_gateway_init_http_onboarding_with_http_upstream() {
    ...
    require(config["mcpServers"]["http-gateway"]["url"] ==
                "http://127.0.0.1:" + std::to_string(gateway_port) + "/mcp/http",
    ...
    require(wait_for_http_gateway(gateway_port, "/mcp/http"),
...
void test_cli_binary_gateway_onboarding_with_stdio_child() {
    ...
    const auto config = Json::parse(config_result->second);
    require(config["mcpServers"]["child-gateway"]["url"] == "http://127.0.0.1:39941/mcp/child",
