...
std::string serialize_test_response(const mcp::protocol::JsonRpcResponse& response) {
    const auto serialized = mcp::protocol::serialize_response(response);
    require(serialized.has_value(), "response should serialize");
    return *serialized;
}

std::string serialize_test_request(const mcp::protocol::JsonRpcRequest& request) {
    const auto serialized = mcp::protocol::serialize_request(request);
    require(serialized.has_value(), "request should serialize");
    return *serialized;
}

std::string serialize_test_notification(const mcp::protocol::JsonRpcNotification& notification) {
...
public:
    explicit RunningServerTransportFixture(std::unique_ptr<mcp::server::HttpTransport> transport,
                                           mcp::server::RequestHandler handler,
                                           mcp::server::NotificationHandler notification_handler = {})
        : transport_(std::move(transport)) {
        thread_ = std::thread([this, handler = std::move(handler), notification_handler = std::move(notification_handler)]() mutable {
            const auto started = transport_->start(std::move(handler), std::move(notification_handler));
            if (!started) {
...
bool wait_for_http_initialize(int port, const std::string& path) {
    const auto initialize_request = serialize_test_request(mcp::protocol::JsonRpcRequest{
    ...
    for (int attempt = 0; attempt < 100; ++attempt) {
        ...
        const auto response = client.Post(path, initialize_headers, initialize_request, "application/json");
...
void test_http_transport_decodes_sse_post_and_answers_interleaved_request() {
    ...
    fixture.server().Post("/mcp", [&](const httplib::Request& request, httplib::Response& response) {
        const auto parsed = mcp::protocol::parse_message(request.body);
        require(parsed.has_value(), "server should parse client message");

        if (const auto* rpc_response = std::get_if<mcp::protocol::JsonRpcResponse>(&*parsed)) {
            ...
            require(rpc_response->result.has_value(), "posted response should contain result");
        ...
        require(rpc_request->method == "tools/list", "server should receive tools/list");

        const auto server_request = serialize_test_request(mcp::protocol::JsonRpcRequest{
        ...
        const auto final_response = serialize_test_response(mcp::protocol::JsonRpcResponse{
            .id = rpc_request->id,
            .result = Json{{"tools", Json::array()}},
        });

        response.set_content(sse_event(server_request) + sse_event(final_response), "text/event-stream");
    });
    ...
    const auto started = transport.start(
        [&](const mcp::protocol::JsonRpcRequest& request) -> mcp::core::Result<mcp::protocol::JsonRpcResponse> {
            ...
            return mcp::protocol::make_response(request.id, Json{{"model", "test-model"}});
        });
    ...
    require(actual->result->at("tools").is_array(), "final response payload mismatch");
    require(interleaved_request_handled.load(), "interleaved server request should be handled");
    require(wait_for([&]() { return client_response_seen.load(); }, std::chrono::milliseconds(1000)),
...
void test_http_transport_opens_get_sse_after_session_and_dispatches_notification() {
    ...
    fixture.server().Post("/mcp", [](const httplib::Request& request, httplib::Response& response) {
        ...
        response.set_content(serialize_test_response(mcp::protocol::JsonRpcResponse{
            ...
            .result = Json{
                ...
                {"serverInfo", Json{{"name", "sse-test"}, {"version", "1"}}},
            },
        }), "application/json");
    });
    fixture.server().Get("/mcp", [&](const httplib::Request& request, httplib::Response& response) {
        ...
        response.set_chunked_content_provider("text/event-stream",
                                              [sent = false, notification](size_t,
    ...
    const auto started = transport.start(
        [](const mcp::protocol::JsonRpcRequest&) -> mcp::core::Result<mcp::protocol::JsonRpcResponse> {
            return std::unexpected(mcp::core::Error{
                static_cast<int>(mcp::protocol::ErrorCode::MethodNotFound),
                "unexpected request",
            });
        },
        [&](const mcp::protocol::JsonRpcNotification& notification) -> mcp::core::Result<mcp::core::Unit> {
            if (notification.method == mcp::protocol::ToolsListChangedNotificationMethod) {
    ...
    require(wait_for([&]() { return get_seen.load(); }, std::chrono::milliseconds(1000)),
            "http transport should open an SSE GET stream");
    require(wait_for([&]() { return notification_seen.load(); }, std::chrono::milliseconds(1000)),
...
void test_http_transport_sets_method_and_name_headers() {
        ...
        if (rpc_request->method == mcp::protocol::InitializeMethod) {
            ...
            response.set_content(serialize_test_response(mcp::protocol::JsonRpcResponse{
                ...
                .result = Json{
                    {"protocolVersion", std::string(mcp::protocol::McpProtocolVersion)},
                    {"capabilities", Json::object()},
                    {"serverInfo", Json{{"name", "headers-test"}, {"version", "1"}}},
                },
            }), "application/json");
            return;
        }

        require(rpc_request->method == mcp::protocol::ToolsCallMethod, "server should receive tools/call");
        require(request.has_header("Mcp-Name"), "tools/call should include Mcp-Name");
        require(request.get_header_value("Mcp-Name") == "echo", "tools/call name header mismatch");
    ...
    fixture.server().Get("/mcp", [&](const httplib::Request& request, httplib::Response& response) {
        if (!request.has_header("Mcp-Session-Id") || request.get_header_value("Mcp-Session-Id") != "header-session") {
            response.status = 404;
            return;
        }
        get_seen.store(true);
    ...
    const auto started = transport.start(
        ...
        [](const mcp::protocol::JsonRpcNotification&) -> mcp::core::Result<mcp::core::Unit> {
            return mcp::core::Unit{};
        });
    require(started.has_value(), "http transport should start");
...
void test_server_http_transport_delivers_outbound_notification() {
    ...
    const auto initialize_response = http_client.Post(kPath,
                                                      ...
                                                      initialize_request,
                                                      "application/json");
    require(static_cast<bool>(initialize_response), "server initialize should succeed");
    require(initialize_response->has_header("Mcp-Session-Id"), "initialize should return a session id");
    const auto session_id = initialize_response->get_header_value("Mcp-Session-Id");

    httplib::Client sse_client("127.0.0.1", kPort);
    httplib::sse::SSEClient sse(sse_client, kPath, httplib::Headers{
    ...
    sse.on_message([&](const httplib::sse::SSEMessage& message) {
        ...
        if (parsed && parsed->method == mcp::protocol::ToolsListChangedNotificationMethod) {
            notification_seen.store(true);
        }
    });
    ...
    const auto sent = server_transport.transport().send_notification(mcp::protocol::JsonRpcNotification{
        .method = std::string(mcp::protocol::ToolsListChangedNotificationMethod),
        .params = Json::object(),
    });
    const std::string sent_error = sent
                                       ? std::string{}
                                       : std::string("server outbound notification should succeed: ") +
                                             sent.error().message +
                                             (sent.error().detail.empty() ? std::string{} : std::string(" | ") + sent.error().detail);
    require(sent.has_value(), sent_error.empty() ? "server outbound notification should succeed" : sent_error);
    require(wait_for([&]() { return notification_seen.load(); }, std::chrono::milliseconds(1000)),
...
void test_server_http_transport_accepts_client_notification_with_202() {
    ...
    RunningServerTransportFixture server_transport(
        ...
        [&](const mcp::protocol::JsonRpcNotification&, const mcp::server::SessionContext& context) {
    ...
    const auto notification_response = http_client.Post(
        ...
        "application/json");
    require(static_cast<bool>(notification_response), "notification should be accepted");
...
void test_server_http_transport_can_request_client() {
    ...
    const auto started = client_transport.start(
        [&](const mcp::protocol::JsonRpcRequest& request) -> mcp::core::Result<mcp::protocol::JsonRpcResponse> {
            if (request.method == mcp::protocol::RootsListMethod) {
                ...
                return mcp::protocol::JsonRpcResponse{
                    .id = request.id,
                    .result = mcp::protocol::roots_list_result_to_json(mcp::protocol::RootsListResult{
            ...
            if (request.method == mcp::protocol::ElicitationCreateMethod) {
                ...
                require(request.params.at("mode") == "url", "elicitation request should preserve mode");
                require(request.params.at("elicitationId") == "elicitation-1",
                        "elicitation request should preserve elicitationId");
                require(request.params.at("url") == "https://example.test/elicitation/1",
        ...
        [&](const mcp::protocol::JsonRpcNotification& notification) -> mcp::core::Result<mcp::core::Unit> {
            if (notification.method == mcp::protocol::ElicitationCompleteNotificationMethod) {
                const auto parsed = mcp::protocol::elicitation_complete_notification_params_from_json(notification.params);
                require(parsed.has_value(), "elicitation completion notification should parse");
    ...
    initialize_params["protocolVersion"] = std::string(mcp::protocol::McpProtocolVersion);
    ...
    initialize_params["clientInfo"] = Json{{"name", "http-transport-test"}, {"version", "1"}};
    ...
    require(peer.supports_sampling_tools(), "client peer should report sampling support");
    require(peer.supports_elicitation(), "client peer should report elicitation support");
    require(peer.supports_elicitation_url(), "client peer should report url elicitation support");
    ...
    require(roots->roots.front().uri == "file:///workspace", "client peer root uri mismatch");
    ...
    require(response->model == "test-model", "client peer create_message model mismatch");
    require(response->content.text == "sampled from client", "client peer create_message text mismatch");
    ...
    const auto elicitation = peer.create_elicitation(mcp::protocol::CreateElicitationRequestParam{
        .message = "open",
        .mode = mcp::protocol::ElicitationMode::Url,
        .elicitation_id = "elicitation-1",
        .url = "https://example.test/elicitation/1",
    });
    require(elicitation.has_value(), "client peer create_elicitation should succeed");
    require(elicitation->action == mcp::protocol::ElicitationAction::Accept,
            "client peer elicitation action mismatch");
    require(!elicitation->content.has_value(), "client peer url elicitation should omit content");

    const auto completion_sent = peer.notify_elicitation_complete("elicitation-1");
    require(completion_sent.has_value(), "client peer notify_elicitation_complete should succeed");

    require(roots_seen.load() == 1, "client should observe one roots/list request");
    require(sampling_seen.load() == 1, "client should observe one sampling request");
    require(elicitation_seen.load() == 1, "client should observe one elicitation request");
    require(wait_for([&]() { return elicitation_complete_seen.load() == 1; }, std::chrono::milliseconds(1000)),
    ...
    const auto reinitialized = client_transport.send(mcp::protocol::JsonRpcRequest{
        .method = std::string(mcp::protocol::InitializeMethod),
        .params = initialize_params,
        .id = std::int64_t{2},
    });
    require(reinitialized.has_value(), "client transport should reinitialize after delete");
    require(reinitialized->result.has_value(), "reinitialize response should contain a result");
    require(reinitialized->result->at("protocolVersion") == std::string(mcp::protocol::McpProtocolVersion),
