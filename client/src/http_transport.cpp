...
bool has_event_stream_content_type(const httplib::Response& response) {
    ...
    return response.get_header_value("Content-Type").find("text/event-stream") != std::string::npos;
}
...
struct HttpTransport::Impl {
    explicit Impl(HttpTransportOptions options)
        : options(std::move(options)) {
        if (this->options.path.empty()) {
            this->options.path = "/";
        }
        if (!this->options.path.starts_with('/')) {
    ...
    core::Result<protocol::JsonRpcResponse> send(const protocol::JsonRpcRequest& request) {
        ...
        while (true) {
            ...
            if (response->status == 404) {
                reset_session();
                if (request.method == protocol::InitializeMethod && !retried_after_session_reset) {
    ...
    core::Result<core::Unit> send_notification(const protocol::JsonRpcNotification& notification) {
        ...
        auto client = make_client();
        auto headers = make_headers(notification.method, std::nullopt, /*json_body=*/true, /*event_stream=*/true);
        const auto response = client.Post(options.path, headers, *serialized, "application/json");
        if (!response) {
            return std::unexpected(make_transport_error(static_cast<int>(protocol::ErrorCode::InternalError),
    ...
    core::Result<core::Unit> ensure_stream_started() {
        {
            ...
            stream_client = std::make_unique<httplib::Client>(options.host, options.port);
            apply_timeout(*stream_client, options.timeout);
            auto headers = to_headers(options.headers);
            headers.emplace("Accept", "text/event-stream");
            headers.emplace("MCP-Protocol-Version", std::string(protocol::McpProtocolVersion));
            headers.emplace(std::string(SessionHeader), session_id);
            sse_client = std::make_unique<httplib::sse::SSEClient>(*stream_client, options.path, headers);
    ...
    core::Result<protocol::JsonRpcResponse> decode_request_response(const httplib::Response& response,
    ...
    core::Result<std::optional<protocol::JsonRpcResponse>> dispatch_event_payload(
        ...
        if (const auto* request = std::get_if<protocol::JsonRpcRequest>(&*message)) {
            ...
            if (!handler) {
                ...
                const auto posted = post_response(response, request->method, header_name_from_request(*request));
                if (!posted) {
    ...
    core::Result<core::Unit> post_response(const protocol::JsonRpcResponse& response,
        ...
        auto client = make_client();
        auto headers = make_headers(method, std::move(name), /*json_body=*/true, /*event_stream=*/false);
        const auto result = client.Post(options.path, headers, *serialized, "application/json");
        if (!result) {
            return std::unexpected(make_transport_error(static_cast<int>(protocol::ErrorCode::InternalError),
...
core::Result<core::Unit> HttpTransport::start(TransportRequestHandler request_handler,
                                              TransportNotificationHandler notification_handler) {
