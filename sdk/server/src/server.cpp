...
namespace mcp::server {
    ...
    namespace {
        ...
        core::Result<core::Unit> update_resource_subscription(
                std::unordered_map<const Transport *, std::unordered_set<std::string>> &subscriptions,
    ...
    core::Result<protocol::JsonRpcResponse> Server::handle_request(const protocol::JsonRpcRequest &request,
        ...
        if (request.method == protocol::InitializeMethod) {
            return protocol::make_response(request.id, initialize_result_to_json(options_));
        }
        ...
        if (request.method == protocol::TasksListMethod) {
            ...
            return protocol::make_response(request.id, protocol::task_list_result_to_json(*result));
        }
        ...
        if (request.method == "resources/subscribe" || request.method == "resources/unsubscribe") {
            ...
            const auto subscription = set_resource_subscription(context,
    ...
    core::Result<core::Unit> Server::handle_notification(const protocol::JsonRpcNotification &notification,
        ...
        if (notification.method == protocol::RootsListChangedNotificationMethod && roots_list_changed_handler_) {
            ...
            const auto cancelled = protocol::cancelled_notification_params_from_json(notification.params);
            if (!cancelled) {
                return std::unexpected(core::Error{
    ...
    core::Result<core::Unit> Server::notify_resource_updated(std::string_view uri) {
        return notify_resource_subscribers(uri, protocol::JsonRpcNotification{
    ...
    core::Result<core::Unit> Server::notify_resource_subscribers(std::string_view uri,
        ...
        bool has_any_subscription = false;
        {
            ...
            for (auto &transport: transports_) {
                ...
                const auto subscription_it = resource_subscriptions_.find(transport.get());
                if (subscription_it == resource_subscriptions_.end()) {
                    continue;
                }
                if (subscription_it->second.find(std::string(uri)) != subscription_it->second.end()) {
    ...
    core::Result<core::Unit> Server::set_resource_subscription(const SessionContext &context,
                                                               std::string_view uri,
                                                               bool subscribed) {
        if (!context.transport) {
            return std::unexpected(core::Error{
    ...
    core::Result<core::Unit> Server::start() {
        for (auto &transport: transports_) {
            const auto started = transport->start([this](const protocol::JsonRpcRequest &request,
                                                         const SessionContext &context) { return this->handle_request(request, context); },
    ...
    ServerBuilder &ServerBuilder::on_task_cancel(Server::TaskCancelHandler handler) {
        task_cancel_handler_ = std::move(handler);
        return *this;
    }

    ServerBuilder &ServerBuilder::on_task_result(Server::TaskResultHandler handler) {
        task_result_handler_ = std::move(handler);
        return *this;
    }
