...
///
/// Server owns registries for MCP tools, prompts, resources, and resource templates.
/// Request handlers return core::Result<T> to surface protocol errors without changing
/// the ABI. Notifications and server-to-client requests are dispatched through the
...
namespace mcp::server {
    ...
    /// @brief Aggregate callback set installable on a Server with Server::set_handler().
    struct ServerHandler;

    /// @brief High-level MCP server facade.
    ///
    /// Server owns the configured transports and registries. Register tools, prompts,
    /// resources, and resource templates before start(), then use transports to serve
    /// JSON-RPC requests. Direct list/get/call/read methods are also available for tests,
    /// embedded use, and custom routing.
    class Server {
    public:
        ...
        /// @param session_id Optional session identifier passed into the tool context.
        ...
        /// @param session_id Optional session identifier passed into the prompt context.
        ...
        /// @param session_id Optional session identifier passed into the resource context.
        ...
        /// @param transport Transport to start when start() is called. It must not be null.
        ...
        /// @brief Handles JSON-shaped extension requests such as completion or sampling.
        ...
        /// @return A response when handled, or std::nullopt to let normal dispatch continue.
        ...
        using TaskListHandler = std::function<core::Result<protocol::TaskListResult>(const protocol::TaskListParams &,
                                                                                     const SessionContext &)>;

        /// @brief Handles task get requests.
        using TaskGetHandler = std::function<core::Result<protocol::Task>(const protocol::TaskGetParams &,
                                                                          const SessionContext &)>;

        /// @brief Handles task cancel requests.
        using TaskCancelHandler = std::function<core::Result<protocol::Task>(const protocol::TaskCancelParams &,
                                                                             const SessionContext &)>;

        /// @brief Handles task result requests.
        using TaskResultHandler = std::function<core::Result<protocol::Json>(const protocol::TaskResultParams &,
                                                                             const SessionContext &)>;
    ...
    private:
        ...
        std::unordered_map<const Transport *, std::unordered_set<std::string>> resource_subscriptions_;
        std::shared_ptr<std::mutex> subscriptions_mutex_ = std::make_shared<std::mutex>();

        core::Result<core::Unit> broadcast_notification(const protocol::JsonRpcNotification &notification);
        core::Result<core::Unit> notify_resource_subscribers(std::string_view uri,
                                                             const protocol::JsonRpcNotification &notification);
        core::Result<core::Unit> set_resource_subscription(const SessionContext &context,
                                                           std::string_view uri,
                                                           bool subscribed);
    };
    ...
    ///
    /// Registrations are accumulated until build(). build() creates a Server, transfers
    /// owned transports/providers, registers tools, prompts, resources, and templates,
    /// then applies configured handler overrides.
    class ServerBuilder {
    public:
        ...
        /// @note The registration is applied during build(); duplicate or invalid names
        /// are reported in the build() result.
        ...
        /// @param resource_template Template metadata advertised by list_resource_templates().
        ...
        /// @return Owned server on success, or the first registration/configuration error.
    ...
    ///
    /// App::builder() exposes a higher-level builder that can create common transports
    /// and adapt simple C++ callables into MCP handlers.
    class App {
    public:
        ...
        public:
            ...
            /// @return Reference to this builder for chaining.
            /// @note std::exception-derived errors thrown by argument decoding are caught
            /// and converted to InvalidParams results when the tool is invoked.
            ...
            /// @param handler Callable accepting Json, string, or no argument and returning
            ...
            /// @param handler Callable accepting ResourceContext or no argument and returning
            /// resource contents/result, protocol::Resource metadata, or core::Result of these.
            template<class Handler>
            ...
            /// @param name Default resource-template name and URI template when the handler
            /// does not fill those fields.
            /// @param handler Callable returning a ResourceTemplate or core::Result<ResourceTemplate>.
            /// @throws std::runtime_error if a Result-returning handler fails during registration.
            template<class Handler>
            ...
            /// @note A handler returning std::optional<JsonRpcResponse> or JsonRpcResponse
            /// controls dispatch; a void-returning handler only observes the request.
            template<class Handler>
