...
/// @file
/// @brief Client transport that launches an MCP server process and talks over stdio.
...
namespace mcp::client {
    ...
    /// @brief Client transport that owns a child process and exchanges JSON-RPC over stdio.
    ///
    /// The transport is move-only through unique ownership by Client. It starts the child
    /// process on demand and routes server-initiated requests and notifications through
    /// callbacks registered with start().
    ...
    public:
        ...
        /// @brief Sends one JSON-RPC request to the child process and waits for a response.
