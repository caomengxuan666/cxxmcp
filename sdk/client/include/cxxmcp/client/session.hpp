...
/// @file
/// @brief Convenience session wrapper around Client initialization and discovery.
...
namespace mcp::client {
    ...
    /// @brief Convenience wrapper that owns a Client and exposes common MCP workflows.
    ///
    /// McpClientSession keeps the low-level Client available while naming common actions
    /// in terms of discovery and session lifecycle. It does not hide the underlying result
    /// model: all operations still return core::Result<T>.
    class McpClientSession {
    public:
        /// @brief Constructs a session from a transport and initialization options.
        /// @param transport Transport owned by the underlying Client. It must not be null.
