...
namespace mcp::client {
    ...
    ///
    /// This transport is useful for tests, embedded integrations, or process environments
    /// where the streams are supplied by the caller instead of launching a child process.
    ...
    public:
        ...
        /// @param output Stream written by the transport. The caller must keep it alive.
        StdioTransport(std::istream &input, std::ostream &output);

        /// @brief Sends one JSON-RPC request over the output stream and waits for a response.
        ...
        /// @param notification_handler Handler for inbound notifications read from the stream.
