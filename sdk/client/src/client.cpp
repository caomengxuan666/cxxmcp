...
namespace mcp::client {
    ...
    namespace {
        ...
        protocol::Json initialize_params(std::string client_name,
                                         std::string client_version,
                                         const std::optional<protocol::ClientCapabilities> &capabilities) {
            protocol::Json params = protocol::Json::object();
            params["protocolVersion"] = std::string(protocol::McpProtocolVersion);
            params["capabilities"] = protocol::client_capabilities_to_json(default_client_capabilities(capabilities));
    ...
    core::Result<protocol::JsonRpcResponse> Client::send_rpc_request(protocol::JsonRpcRequest request) {
        ...
        if (request.method != std::string(protocol::InitializeMethod) && !last_initialize_params_.has_value() &&
    ...
    core::Result<protocol::Json> Client::initialize(std::string client_name, std::string client_version) {
        auto params = initialize_params(std::move(client_name), std::move(client_version), capabilities_);
    ...
    core::Result<protocol::Json> Client::task_result(const protocol::TaskResultParams &request) {
        return send_request(std::string(protocol::TasksResultMethod), protocol::task_result_params_to_json(request));
    }
